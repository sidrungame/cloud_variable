...
/** Rooms with a save pending, and their debounce timer. @type {Map<string, NodeJS.Timeout>} */
const pendingSaves = new Map();
/** Delay before writing a changed room to persistence, to coalesce rapid changes. */
const SAVE_DEBOUNCE_MS = 2000;

/**
 * Schedule a debounced save of a room's current variables to persistence.
...
function scheduleSave(room) {
  ...
  const timer = setTimeout(() => {
    ...
    persistence.saveRoom(room.id, variables);
  }, SAVE_DEBOUNCE_MS);
...
wss.on('connection', (ws, req) => {
  ...
  async function performHandshake(roomId, username) {
    ...
    if (rooms.has(roomId)) {
      ...
      // Room didn't exist in memory yet; see if we have persisted data for it.
      const saved = await persistence.loadRoom(roomId);
      if (saved) {
        /** @type {string[]} */
        const messages = [];
        for (const [name, value] of Object.entries(saved)) {
          if (!room.has(name) && validators.isValidVariableName(name) && validators.isValidVariableValue(value)) {
